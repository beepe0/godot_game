...
public partial class DungeonResourceLoader : ComponentObject
{
    ...
    public void LoadResources()
    {
        try
        {
            ...
            foreach (var preset in DungeonBuilder.DungeonBuilderPreset.CategoryScenes)
            {
                ...
                if (roomsFolder != null)
                {
                    ...
                    while (filename != "")
                    {
                        if (!roomsFolder.CurrentIsDir())
                        {
                            ...
                            preset.PackedScenes.Add(ResourceLoader.Load<PackedScene>(roomPath));
                
                            GameConsole.Instance.DebugLog($"{GetType()} :: Loaded room at {GameConsole.SetColor(roomPath, "#7db39e")}, Filename: {GameConsole.SetColor(filename, "#7db39e")}, Category: {GameConsole.SetColor(preset.TilesCategory, "#7db39e")}");
